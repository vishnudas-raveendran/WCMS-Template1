...
if(isset($_POST['submit']) && $_POST['submit']=="insert"){
 ...
 if($_FILES['image']['size']==0)
 {
  ...
  header('Location:../presentation/news.php');
...
$sql="INSERT INTO `ce`.`news` (`title`, `caption`,`link`, `Img_ref`) VALUES (?,?,?,?)";
$stmt=$db->prepare($sql);
...
// echo $savein;
 ...
 header('Location:../presentation/news.php'); 
	 ...
	 $sql="UPDATE `news` SET `title`=?,`caption`=?,`link`=? WHERE `id`=?";
    $stmt=$db->prepare($sql);
    if($stmt->execute(array(htmlentities($_POST['title']),htmlentities($_POST['caption']),htmlentities($_POST['link']),htmlentities($_POST['update'])))){
		...
		header('Location:../presentation/news.php');
				...
				$dir="../../images/news";
				 ...
				 $sql="SELECT `img_ref` FROM news WHERE `id`=?";
			$stmt=$db->prepare($sql);
			$stmt->execute(array(htmlentities($_POST['update'])));
			$e=$stmt->fetch();
			$path="../../images/news/".$e['img_ref'].".jpg";
			if(unlink ($path)){ echo "previous image deleted";}else{ echo "Error deleting previous image";}  
				...
				$rpic=md5($pic);
				 ...
				 if(move_uploaded_file($_FILES['image']['tmp_name'],$savein))
					...
					$sql="UPDATE `news` SET `img_ref`=? WHERE `id`=?";
					$stmt=$db->prepare($sql);
